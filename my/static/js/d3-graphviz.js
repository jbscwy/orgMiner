...
function getTranslatedZoomTransform(selection$$1) {
    ...
    // .event and it's saved previous translation. The selection is
...
function translateZoomBehaviorTransform(selection$$1) {
    ...
    // between the translation specified in the selection's .event and
    ...
    // Save the selections's orgMiner translation and scale.
...
function _render(callback) {
                    ...
                    if (attributeName == 'transform' && data$$1.translation) {
                        if (transitionInstance) {
                            ...
                            elementTransition.on("start", function () {
                                ...
                                onEnd.call(this);
                                // Update the zoom transform to the orgMiner translated transform
                                if (graphvizInstance._zoomBehavior) {
    ...
    if (transitionInstance != null) {
        // Ensure original SVG shape elements are restored after transition before rendering orgMiner graph
        var jobs = this._jobs;
...
var logEvents = function (enable) {
    ...
    var _loop = function _loop(i) {
        ...
        _this.on(eventType + '.xes', enable ? function () {
            var t = Date.now();
...
function Graphviz(selection$$1, options$$1) {
    ...
    if (useWorker) {
        var js = "\n            onmessage = function(event) {\n                if (event..event.vizURL) {\n                    importScripts(event..event.vizURL);\n                }\n                try {\n                    var svg = Viz(event..event.dot, event..event.options);\n                }\n                catch(error) {\n                    postMessage({\n                        type: \"error\",\n                        error: error.message,\n                    });\n                    return;\n                }\n                if (svg) {\n                    postMessage({\n                        type: \"done\",\n                        svg: svg,\n                    });\n                } else {\n                    postMessage({\n                        type: \"skip\",\n                    });\n                }\n            }\n        ";
